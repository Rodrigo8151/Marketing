...
const filters = ['Flagship Stores', 'Outlets', 'Distribuidores Oficiales'];
const legendItems = [
  { name: 'Flagship Store', color: '#E6397A' },
  { name: 'Outlet', color: '#4285F4' },
  { name: 'Distribuidor', color: '#FBBC05' },
];
    ...
    <section id="map" className="map-container">
      ...
      <div className="map-filters">
        {filters.map((filter, index) => (
          <button key={filter} className={index === 0 ? 'active' : ''}>
            {filter}
          </button>
        ))}
      </div>
